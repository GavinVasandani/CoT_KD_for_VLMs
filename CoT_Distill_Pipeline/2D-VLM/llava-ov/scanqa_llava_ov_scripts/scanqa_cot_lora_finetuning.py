...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, data_path, images_dir, processor, prompt_template, visual_obs_template, question_template, df, teacher_cache_dir, split, max_length=512):
    ...
    def __getitem__(self, idx):
        ...
        question_prompt = self.question_template.format(
            model_vis_obs="<VISUAL_OBS>", change="",
        ...
        full_prompt = vis_input + ans_input
        ...
        prompt_len = self.processor.tokenizer(vis_input+ans_input, return_tensors="pt")["input_ids"].size(1)
        ...
        if self.split == "train":
            ...
            teacher_logits = torch.cat([vis, ans], dim=0)
...
def main():
    ...
    train_dataset = Dataset(args.train_data, args.images_dir, processor, prompt_template, visual_obs_template, question_template, df, args.teacher_cache_dir, split="train")
    eval_dataset = Dataset(args.eval_data, args.images_dir, processor, prompt_template, visual_obs_template, question_template, df, args.teacher_cache_dir, split="eval")
    ...
    print(f"Distilled LoRA adapter saved to: {full_path}")
