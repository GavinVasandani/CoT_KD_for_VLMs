...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        full_text = full_prompt + " " + sample["answer"]
        
        tokenized = self.processor.tokenizer(
            full_prompt,                    # first sequence
            sample["answer"],              # second sequence
            return_tensors="pt",
            padding="max_length",
        ...
        #  tokenize the prompt to get its true length in order to apply masking up till prompt length
        prompt_ids = self.processor.tokenizer(full_prompt, return_tensors="pt", truncation=True, max_length=self.max_length)["input_ids"].squeeze(0)
        prompt_len = prompt_ids.size(0)

        # mask only the prompt portion
...
def main():
    ...
    # find all qkv projection layers to apply lora adapters to
