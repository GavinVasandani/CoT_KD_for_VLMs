# Script that does normal model inference. Uses baseline model for inference - no LoRA adapter.
# Outputs single answers, no CoT or raw_logits.
...
def is_ambiguous_choice_question(question):
    """
    Returns True if question contains ' or ' which typically indicates a binary choice
    like 'Is it on the left or right?', which are guessable.
    """
    lowered = question.lower()
    return " or " in lowered
...
for scene_id, changes_list in list(data.items()):
    for i, changes in enumerate(changes_list):
        ...
        for j, qa in enumerate(question_answers):
            ...
            if is_ambiguous_choice_question(question):
                print("The QUESTION IS SKIPPED")
                continue
            ...
            # might have to scale max_new_tokens with model size so if using llava-ov 72b then scale max_new_tokens to 512?
...
output_path = f"{output_dir}/blind_test_evaluation_overall_metrics_{model_name}_without_cot_{timestamp}.txt"
