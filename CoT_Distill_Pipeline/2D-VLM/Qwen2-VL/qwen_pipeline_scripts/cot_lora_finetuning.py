...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # question prompt
        question_prompt = self.question_template.format(model_vis_obs="<VISUAL_OBS>", change=change, context_change_obs="<CHANGE_OBS>", question=question)
...
class DistillTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if teacher_logits is not None:
            ...
            kl = F.kl_div(s_log_probs, t_probs, reduction="none")
            mask = (labels != -100).float().unsqueeze(-1)
            kl = kl * mask  # still differentiable
            kl_loss = kl.sum() / mask.sum() * (T * T)
        else:
            kl_loss = torch.zeros_like(ce_loss, requires_grad=True)
...
def main():
    ...
    print("Trainable modules and requires_grad flags:")
    ...
    training_args = TrainingArguments(
        ...
        report_to="wandb",
        run_name="CoTLoRAFinetuningQWEN",
        label_names=["labels"],
        # optim="adamw_torch",
    ...
    print(f"Number of trainable parameters after injecting LoRA: {num_train_params}")
