...
mkdir -p "$pipeline_run_dir" - commented to use existing dir
...
if [[ -f "$data_path" ]]; then
    ...
    # this will give qwen 2b zero-shot
    echo "Running inference for zero-shot student model on test split to see zero-shot model performance"
    echo "Model dir path is: ${model_dir_path}"
    # need to make bertscore_evaluate for llava-ov so just rename llava-ov bertscore_evaluate to evaluate
    python 2D-VLM/$model_dir_path/evaluate.py \
        -f "$cot_test_data_path" \
        -m "$student_model_id" \
        -p "$pipeline_run_dir" \

    # this will give qwen 2b std lora finetuned adapter - already done
    ...
    # this will give qwen 2b std lora finetuned adapter performance vals
