...
if [[ -f "$data_path" ]]; then
    ...
    python 2D-VLM/llava-ov/scanqa_evaluate.py \
    ...
    python 2D-VLM/llava-ov/scanqa_normal_lora_finetuning.py \
    ...
    echo "----STANDARD LORA FINETUNED TEACHER INFERENCE ON TEST SET:-----"

    echo "Base/Zero-shot teacher model with standard finetuned LoRA Adapter performance evaluation on test split"
    echo "The teacher standard lora adapter is ${teacher_standard_lora_adapter}"
    python 2D-VLM/llava-ov/scanqa_model_with_lora_evaluate.py \
        -f "$cot_test_data_path" \
        -m "$teacher_model_id" \
    ...
    python 2D-VLM/llava-ov/scanqa_cot_gen.py \
    ...
    python 2D-VLM/llava-ov/scanqa_cot_lora_finetuning.py \
        -f "$cot_gen_data_path" \
        -m "$student_model_id" \
        -i "$images_dir" \
        -e "$cot_val_data_path" \
    ...
    echo "The student standard lora adapter is ${student_standard_lora_adapter}"
    python 2D-VLM/llava-ov/scanqa_model_with_lora_evaluate.py \
        -f "$cot_test_data_path" \
        -m "$student_model_id" \
        -a "$student_standard_lora_adapter" \
        -p "$pipeline_run_dir" 

    echo "Student model with LoRA adapter (finetuned on CoT + raw_logits from teacher model) performance evaluation"
    echo "The student cot lora adapter is ${student_cot_lora_adapter}"
    python 2D-VLM/llava-ov/scanqa_model_with_lora_evaluate.py \
        -f "$cot_test_data_path" \
        -m "$student_model_id" \
