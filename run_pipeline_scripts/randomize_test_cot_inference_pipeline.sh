...
if [[ -f "$data_path" ]]; then
   ...
   echo "Running baseline student model (llava-ov-0.5b) to see baseline model performance"
   python 2D-VLM/llava-ov/evaluate.py \
	    -f "$val_data_path" \
	    -m "llava-hf/llava-onevision-qwen2-0.5b-ov-hf" \

    echo "Running Student Model (llava-ov-0.5b) with LoRA adapter (finetuned on CoT + raw_logits from teacher model)"
