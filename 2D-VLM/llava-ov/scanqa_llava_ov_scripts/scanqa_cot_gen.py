...
images_dir = "/gpfs/home/ym621/gavin/Hypo3D/ScanQA_data/all_scannet_imgs"

SYSTEM_INSTRUCTIONS = """
You are a visual reasoning assistant.

When I ask a question, you must consider the scene, then give an answer.
...
visual_obs_template = '''
Given a top-view of a 3D scene describe everything you see, listing each object and its location relative to the scene’s center (e.g. “The couch is at the back‐left corner, the plant is near the front‐right edge).
'''
...
for scene_id, changes_list in tqdm(list(data.items())):
    ...
    # Phase 1: visual
    model_vis_obs, vis_obs_token_ids, vis_obs_logits = run_model_inference("visual", orientation="")
    ...
    for i, changes in tqdm(enumerate(changes_list)):
        ...
        for j, qa in tqdm(enumerate(question_answers)):
            ...
            question_type = get_sub_question_type(question)
            answer = qa['answers'][0]
            ...
            torch.save({
            "token_ids": vis_obs_token_ids, 
            "logits":    vis_obs_logits,
            }, f"{args.pipeline_run_dir}/teacher_cache/{overall_qs_idx}/visual_observations.pt")

            # Store context_change logits
            # os.makedirs(f"{args.pipeline_run_dir}/teacher_cache/{overall_qs_idx}", exist_ok=True)
            # torch.save({
            # "token_ids": change_obs_token_ids, 
            # "logits":    change_obs_logits,
            # }, f"{args.pipeline_run_dir}/teacher_cache/{overall_qs_idx}/context_change.pt")
            ...
            }, f"{args.pipeline_run_dir}/teacher_cache/{overall_qs_idx}/answer.pt")
