...
def save_json(data, filename, pipeline_run_dir):
    ...
    print(f"Saved predictiosn to {full_path}")
...
for scene_id, changes_list in list(data.items()):
    ...
    # Not really used
    save_json(data, f"{args.model_id.split('/')[1]}_no_label_align_cot.json", args.pipeline_run_dir)
...
adapter_name = os.path.basename(args.adapter_dir).replace("/", "_")
results_path = os.path.join(args.pipeline_run_dir, f"{model_name}_with_{adapter_name}_evaluation_metrics.txt")
